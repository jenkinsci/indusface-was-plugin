...
public class ScanApiLaunch {
			...
			while (isScanRunning) {
				...
				try {
					...
					if (scanApiResponse.getScanStatus().equals("Completed")) {
						listener.getLogger().println("Scan complete. View report now.Current Build Status: "
					...
					listener.getLogger().println(
							"Scanning in progress. Checking status in 5 minutes.: " + scanApiResponse.getScanStatus());
	...
	private HttpResponse<String> startScan(String accessKey)
		...
		HttpRequest request = HttpRequest.newBuilder().uri(scanUri)
				.header("Content-Type", "application/x-www-form-urlencoded")
				.POST(HttpRequest.BodyPublishers.ofString(secretKey)).build();
